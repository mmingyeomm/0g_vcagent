...
export default function Create() {
  ...
  useEffect(() => {
    ...
    const totalFields = 7; // name, description, risk, focus, min, max, details
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col space-y-2 mb-8">
          ...
          <p className="text-gray-400">
            Define your investment profile and preferences
          </p>
        </div>
                      ...
                      {investmentFocusOptions.map((option, index) => (
                        <div
                          ...
                          className={`relative rounded-lg border border-[#3A3A3A] hover:border-[#4A4A4A] bg-[#2A2A2A] p-3 cursor-pointer transition-all duration-200 ${
                            watch(`investmentFocus.${index}`)
                              ? "bg-[#2A2A2A] border-[#00F5A0]"
                              : ""
                          }`}
                          onClick={() => {
                            const currentValue = getValues(
                              `investmentFocus.${index}`
                            );
                            setValue(
                              `investmentFocus.${index}`,
                              !currentValue as any
                        ...
                        >
                          <div className="flex items-start">
                            <div
                              className={`h-4 w-4 rounded border border-[#4A4A4A] flex items-center justify-center mr-2 mt-0.5 ${
                                watch(`investmentFocus.${index}`)
                                  ? "bg-[#00F5A0] border-[#00F5A0]"
                                  : ""
                              }`}
                            >
                              {watch(`investmentFocus.${index}`) && (
                ...
                <div className="space-y-6 animate-float">
                  <div>
                    ...
                    <div className="flex items-center mb-2">
                      <DocumentTextIcon className="h-5 w-5 text-[#00F5A0] mr-2" />
                      <span className="text-sm text-gray-300">
                        Add any additional information about this investor
                        profile
                      </span>
                    </div>
                    <div className="relative group">
                      ...
                      <textarea
                        ...
                        className="block w-full rounded-lg bg-[#2A2A2A] border border-[#3A3A3A] px-4 py-3 text-white focus:border-[#00F5A0] focus:ring focus:ring-[#00F5A0]/20 sm:text-sm transition-all duration-200"
                        placeholder="Add more detailed information about this investor (e.g., preferred markets, historical performance, investment strategy details)"
                      />
                    </div>
                    {errors.details && (
                      <p className="mt-1 text-sm text-red-400">
